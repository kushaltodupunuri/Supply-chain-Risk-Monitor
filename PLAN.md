...
- [x] **Reverted:** tried renaming `app.py` → `Risk_Monitor.py` so the sidebar would show "Risk Monitor" instead of the filename-derived "app" entry. This broke the live Streamlit Cloud deployment - newer versions of Streamlit Cloud don't expose "Main file path" as an editable setting after the app is first created, so there was no way to point the existing deployment at the new filename without deleting and recreating the app. Reverted back to `app.py` to restore the live app; the nav label is still "app" until solved a different way (e.g. the newer `st.navigation`/`st.Page` API, which can set a custom title without depending on the entry script's filename).
...
| Post-Launch — SupplyIQ: 6 new operational modules | Done | Sidebar still shows "app" for the main page - rename attempt was reverted, see notes above |
