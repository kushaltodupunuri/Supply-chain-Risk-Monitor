...
## Post-Launch: SupplyIQ — 6 New Operational Modules
- [x] Added 6 new sidebar pages under `pages/` (Streamlit's native multipage auto-discovery, zero wiring needed in `app.py`), collectively branded **SupplyIQ** — a separate, small-business-facing toolset that works from data the user enters/uploads rather than live external feeds: **Supply Chain Simulator** (5-question flow diagram with heuristic health scoring), **Demand Forecasting Engine** (Moving Average + Exponential Smoothing + Holt-Winters, with seasonal/stockout/overstock alerts), **Supplier Performance Scorecard** (weighted 0-100 score, A-F grades, cost-of-poor-performance estimate), **Logistics & Route Optimizer** (carrier cost comparison, consolidation detection, carbon estimate), **Cost Optimization Engine** (pulls live `st.session_state` data from the Scorecard and Logistics pages, totals savings across 4 categories), and **Executive Dashboard** (aggregates all 5 other modules into one view with prioritized action items).
- [x] **Substituted Facebook Prophet with statsmodels Holt-Winters** in the Demand Forecasting module — Prophet's build chain (cmdstanpy/pystan) is heavy and known to be fragile on Windows; Holt-Winters covers the same trend+seasonality capability with a much lighter, more reliable dependency.
- [x] **No real carrier-rate API in the Logistics module** — FedEx/UPS/USPS/DHL all require a business shipping account for real quotes, not a free public API. Built a hand-curated, clearly-labeled approximate cost model instead (same "honest baseline" pattern as the Risk Monitor's `shipping.py`), with an explicit on-page warning that it's for relative comparison, not real quotes.
- [x] Added `src/ui_helpers.py` for shared card/CSS styling across the 6 new pages, since each Streamlit multipage script runs independently and doesn't inherit `app.py`'s CSS.
- [x] **Reverted, then fixed differently:** first tried renaming `app.py` → `Risk_Monitor.py` so the sidebar would show "Risk Monitor" instead of the filename-derived "app" entry. This broke the live Streamlit Cloud deployment - newer versions of Streamlit Cloud don't expose "Main file path" as an editable setting after the app is first created, so there was no way to point the existing deployment at the new filename without deleting and recreating the app. Reverted back to `app.py` to restore the live app, then solved the actual goal a different way: moved all of `app.py`'s content into `risk_monitor_view.py`, and turned `app.py` into a thin router that calls `st.navigation([st.Page("risk_monitor_view.py", title="Risk Monitor", ...), st.Page("pages/1_...", ...), ...]).run()`. This sets a custom sidebar title independent of any filename, so `app.py` itself never has to change - the Cloud deployment's pinned entry point stays untouched no matter what. Required removing the per-page `st.set_page_config()` calls from `risk_monitor_view.py` and all 6 `pages/*.py` files, since `st.navigation` only allows one call total, made once in `app.py` before `st.navigation()` - the trade-off is that the browser tab title/favicon no longer changes per page (always shows `app.py`'s "Risk Monitor - SupplyIQ"), which only affects the browser tab, not the sidebar.
...
| Post-Launch — SupplyIQ: 6 new operational modules | Done | |
| Post-Launch — "Risk Monitor" sidebar label via st.navigation (app.py unchanged) | Done | See notes above - first attempt (renaming app.py) broke the live deploy, second attempt fixed it without renaming anything Cloud depends on |
