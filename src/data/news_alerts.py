...
def _risk_query(subject_expr):
    """Wraps an already-formed query expression with the risk keyword clause.
    ...
    """
    quoted_keywords = [f'"{kw}"' if " " in kw else kw for kw in RISK_KEYWORDS]
    keyword_clause = " OR ".join(quoted_keywords)
    return f'({subject_expr}) AND ({keyword_clause})'


def get_news_risk_count(subject_expr, cache_key, days=7):
    """Counts recent news articles matching subject_expr alongside risk keywords.
    ...
    params = {
        ...
        "qInTitle": _risk_query(subject_expr),
        "language": "en",
        "from": from_date,
        "sortBy": "publishedAt",
        "pageSize": 10,
...
def get_relative_alert(subject_expr, cache_key, recent_days=7, baseline_days=30):
    ...
    """
    recent = get_news_risk_count(subject_expr, cache_key, days=recent_days)
    last_30 = get_news_risk_count(subject_expr, cache_key, days=baseline_days)
...
def get_country_alert(country_name, recent_days=7, baseline_days=30):
    ...
    return get_relative_alert(subject_expr, country_name, recent_days, baseline_days)
...
def get_route_alert(route_name, recent_days=7, baseline_days=30):
    ...
    return get_relative_alert(subject_expr, route_name, recent_days, baseline_days)
