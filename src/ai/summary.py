...
"""
    ...
    try:
        start, end = raw.index("{"), raw.rindex("}") + 1
        data = json.loads(raw[start:end])
        result = {"known": bool(data.get("known", False)), "reasoning": str(data.get("reasoning", ""))}
        for field in fields:
            result[field] = max(-15, min(15, float(data.get(field, 0))))
        return result
    except Exception:
        result = {"known": False, "reasoning": ""}
        for field in fields:
            result[field] = 0
        return result
...
RECOMMENDATION_LIBRARY = {
    ...
    "regulatory": {
        ...
        "detail_template": (
            "Audit HTS/tariff classifications across your {industry} import categories and confirm "
            "you're utilizing any applicable free trade agreements. Assign someone to track trade "
